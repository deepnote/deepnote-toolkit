...
print("Loaded jupyter_server_config.py")

# get_config() is defined in the global namespace by the Jupyter config module
...
#          limited.
#  Default: 1000000
# NOTE: raising this to 15MB because we sometimes need to get huge chunk of data from
# the kernel into webapp (e.g. when loading data previews). For other
# executions (e.g. initiated by user), we trim output sent to browser
# in the executor, so there is no risk of sending too much data to the browser (which
# is the original purpose of this option in Jupyter)
c.ServerApp.iopub_data_rate_limit = 1024 * 1024 * 15  # 15MB
...
## Enable jupyter_server_terminals extension for terminal support
# and jupyter_resource_usage for resource usage tracking through API
...
#  Default: 8888
c.ServerApp.port = int(os.getenv("DEEPNOTE_JUPYTER_PORT", 8888))
...
#          with the DEEPNOTE_JUPYTER_TOKEN environment variable.
#
...
#  is NOT RECOMMENDED.
#  Default: '<generated>'
c.ServerApp.token = os.getenv("DEEPNOTE_JUPYTER_TOKEN", "")
...
#  kill may be equivalent on windows.  Note that this value can be overridden by
#  the in-use kernel provisioner since shutdown times may vary by provisioned
#  environment.
#  Default: 5.0
...
## The name of the unpacker for deserializing messages.
...
c.MultiKernelManager.default_kernel_name = (
    os.environ["DEFAULT_KERNEL_NAME"]
...
#  Default: True
# TODO: With new execution architecture it is safe to disable this
...
#  Choices: any of ['sha1', 'blake2b', 'blake2s', 'sha3_224', 'sha384', 'sha512', 'sha3_384', 'sha3_512', 'sha224', 'sha256', 'md5', 'sha3_256']
#  Default: 'sha256'
...
# ------------------------------------------------------------------------------
# ResourceUseDisplay configuration
# ------------------------------------------------------------------------------
## Enable CPU usage tracking in jupyter-resource-usage
